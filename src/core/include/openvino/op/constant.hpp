...
public:
    ...
    std::vector<T> cast_vector(int64_t num_elements = -1) const {
        ...
        if constexpr (data::is_supported_type<T>) {
            ...
            std::vector<long long> rc(num_elements_to_cast);
            data::cast_n(m_element_type, get_data_ptr(), num_elements_to_cast, rc.begin());
            return {rc.begin(), rc.end()};
        } else if constexpr (std::is_unsigned_v<T>) {
            std::vector<unsigned long long> rc(num_elements_to_cast);
            data::cast_n(m_element_type, get_data_ptr(), num_elements_to_cast, rc.begin());
            return {rc.begin(), rc.end()};
        } else {
            std::vector<double> rc(num_elements_to_cast);
            data::cast_n(m_element_type, get_data_ptr(), num_elements_to_cast, rc.begin());
            return {rc.begin(), rc.end()};
        }
    }
...
private:
    ...
    void write_values(const std::vector<T>& source) {
        ...
        if constexpr (data::is_supported_type<T>) {
            ...
            std::vector<long long> temp{source.begin(), source.end()};
            data::copy_n(m_element_type, temp.begin(), temp.size(), get_data_ptr_nc());
        } else if constexpr (std::is_unsigned_v<T>) {
            std::vector<unsigned long long> temp{source.begin(), source.end()};
            data::copy_n(m_element_type, temp.begin(), temp.size(), get_data_ptr_nc());
        } else {
            std::vector<double> temp{source.begin(), source.end()};
            data::copy_n(m_element_type, temp.begin(), temp.size(), get_data_ptr_nc());
        }
    }
    ...
    struct Data {
        ...
        using const_ptr = std::variant<const Ts*...>;
        using const_iter = std::variant<typename std::vector<Ts>::const_iterator...>;
        using iter = std::variant<typename std::vector<Ts>::iterator...>;
        ...
        static void copy_n(const element::Type& type, const const_iter& src, const size_t n, void* dst);
        static void cast_n(const element::Type& type, const void* src, const size_t n, const iter& dst);
    };
...
template <>
OPENVINO_API void Constant::data::copy_n(const element::Type& type, const const_iter& src, const size_t n, void* dst);

template <>
OPENVINO_API void Constant::data::cast_n(const element::Type& type, const void* src, const size_t n, const iter& dst);
