...
OPENVINO_API void Constant::data::copy_n(const element::Type& type, const const_iter& src, const size_t n, void* dst) {
    const auto visitor = ov::util::VariantVisitor{
        ...
        [&type, dst, n](const std::vector<std::string>::const_iterator src) {
            if (type == element::string) {
...
OPENVINO_API void Constant::data::cast_n(const element::Type& type, const void* src, const size_t n, const iter& dst) {
    const auto visitor = ov::util::VariantVisitor{
        [&type, src, n](const auto& dst) {
            if (type != element::string) {
                using namespace ov::element;
                using dst_type = typename std::iterator_traits<std::decay_t<decltype(dst)>>::value_type;
                IfTypeOf<SUPPORTED_ET>::apply<ElementConvert<dst_type>>(type, src, dst, n);
            } else {
                fill_error_msg(type, element::from<decltype(*dst)>());
            }
        },
        [&type, src, n](const std::vector<std::string>::iterator dst) {
            if (type == element::string) {
